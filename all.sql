...
template_query_02	
with wscs as
  (select /*+CONCURRENT_TASK_RATIO(2)*/ 
  sold_date_sk
  ,sales_price
  ...
  from catalog_sales)),
  wswscs as 
  (select /*+CONCURRENT_TASK_RATIO(2)*/ d_week_seq,
  ...
  group by d_week_seq)
  select /*+CONCURRENT_TASK_RATIO(2)*/ d_week_seq1
...
template_query_05	
with ssr as
  (select /*+CONCURRENT_TASK_RATIO(2)*/ s_store_id,
  ...
  ,
  csr as
  (select /*+CONCURRENT_TASK_RATIO(2)*/ cp_catalog_page_id,
  sum(sales_price) as sales,
  sum(profit) as profit,
  sum(return_amt) as returns,
  sum(net_loss) as profit_loss
  ...
  ,
  wsr as
  (select /*+CONCURRENT_TASK_RATIO(2)*/ web_site_id,
  sum(sales_price) as sales,
  sum(profit) as profit,
  sum(return_amt) as returns,
  sum(net_loss) as profit_loss
...
select 
  ...
  t.cd_dep_count,
  count(*) cnt4,
  t.cd_dep_employed_count,
  count(*) cnt5,
  t.cd_dep_college_count,
  count(*) cnt6
  from
 (
  select
...
template_query_11	
with year_total as (
  select /*+CONCURRENT_TASK_RATIO(2)*/ c_customer_id customer_id
  ,c_first_name customer_first_name
  ,c_last_name customer_last_name
  ,c_preferred_cust_flag customer_preferred_cust_flag
  ,c_birth_country customer_birth_country
...
select avg(ss_quantity)
 ...
 ( cd_marital_status = 'U'
  ...
  ( ca_state in ('UT', 'OK', 'IL')
  and ss_net_profit between 150 and 300 
  ) or
  ( ca_state in ('AL', 'NE', 'GA')
...
select * from (select sum(cs_ext_discount_amt) as ""excess discount amount""
 from 
  catalog_sales 
  join date_dim 
  on (d_date between date'1999-02-22' and 
  (cast('1999-02-22' as date) + 90 )
  and d_date_sk = cs_sold_date_sk 
  
  )
  join item 
  on(i_manufact_id = 283
...
select 
  ...
  t.cd_dep_count,
  count(*) cnt1,
  stddev_samp(t.cd_dep_count),
  max(t.cd_dep_count),
  min(t.cd_dep_count),
  t.cd_dep_employed_count,
  count(*) cnt2,
  stddev_samp(t.cd_dep_employed_count),
  max(t.cd_dep_employed_count),
  min(t.cd_dep_employed_count),
  t.cd_dep_college_count,
  count(*) cnt3,
  stddev_samp(t.cd_dep_college_count),
  max(t.cd_dep_college_count),
  min(t.cd_dep_college_count)
  from
  (
  select
  ca_state,
...
select 
  ...
  ,rank_within_parent
  limit(1,100);"
template_query_37	"select i_item_id
  ,i_item_desc
  ,i_current_price
  ...
  order by i_item_id
  limit(1,100);"
template_query_38	"select count(*) from (
...
template_query_47	
with v1 as(
  select /*+CONCURRENT_TASK_RATIO(2)*/ i_category, i_brand,
  ...
  limit(1,100);
template_query_48	
select /*+CONCURRENT_TASK_RATIO(2)*/ sum (ss_quantity)
  from store_sales
  , date_dim
  , store
  , customer_demographics
...
select /*+MULTI_AGG*/
 ...
 from
  store_sales
  ,(store_returns join date_dim d2 on d2.d_year = 1999
 and d2.d_moy = 10
 and sr_returned_date_sk = d2.d_date_sk)
  ,store
  ,date_dim d1 
...
with ss as (
 ...
 ,sum(total_sales) total_sales
  ...
  limit(1,100);
template_query_61	
select promotions,total,cast(promotions as decimal(15,4))/cast(total as decimal(17,4))*100
 from
...
with year_total as (
  ...
  and t_s_secyear.sale_type = 's'
  and t_w_secyear.sale_type = 'w'
  and t_s_firstyear.year1 = 1998
  and t_s_secyear.year1 = 1998+1
  and t_w_firstyear.year1 = 1998
  and t_w_secyear.year1 = 1998+1
  and t_s_firstyear.year_total > 0
  and t_w_firstyear.year_total > 0
